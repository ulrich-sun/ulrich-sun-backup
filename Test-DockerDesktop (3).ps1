...
param(
    ...
    [string] $ImportTar,
    [string] $Shell,
    [ValidateSet('posix','pwsh','cmd')] [string] $ShellKind = 'posix'
)
...
# ===========================================================================
#  COUCHE D'EXECUTION AGNOSTIQUE
#  Le banc d'essai ne suppose plus /bin/sh : il detecte l'interpreteur
#  reellement present dans l'image (sh, bash, ash, busybox, powershell.exe,
#  cmd.exe) et traduit chaque charge de test dans la syntaxe correspondante.
# ===========================================================================

$script:ShExe    = '/bin/sh'
$script:ShArgs   = @('-c')
$script:ShKind   = 'posix'      # posix | pwsh | cmd
$script:PathVol  = '/vol'
$script:PathBind = '/bind'

$script:ShellCandidates = @(
    @{ Kind='posix'; Exe='/bin/sh';             Args=@('-c') },
    @{ Kind='posix'; Exe='/bin/bash';           Args=@('-c') },
    @{ Kind='posix'; Exe='/bin/ash';            Args=@('-c') },
    @{ Kind='posix'; Exe='/bin/dash';           Args=@('-c') },
    @{ Kind='posix'; Exe='/usr/bin/sh';         Args=@('-c') },
    @{ Kind='posix'; Exe='/usr/bin/bash';       Args=@('-c') },
    @{ Kind='posix'; Exe='/usr/local/bin/bash'; Args=@('-c') },
    @{ Kind='posix'; Exe='/busybox/sh';         Args=@('-c') },
    @{ Kind='posix'; Exe='/bin/busybox';        Args=@('sh','-c') },
    @{ Kind='pwsh';  Exe='powershell.exe';      Args=@('-NoProfile','-NonInteractive','-Command') },
    @{ Kind='pwsh';  Exe='pwsh.exe';            Args=@('-NoProfile','-NonInteractive','-Command') },
    @{ Kind='cmd';   Exe='cmd.exe';             Args=@('/c') }
)

function Set-Runner {
    param([hashtable]$Cand)
    $script:ShExe  = $Cand.Exe
    $script:ShArgs = $Cand.Args
    $script:ShKind = $Cand.Kind
    if ($Cand.Kind -eq 'posix') {
        $script:PathVol = '/vol' ; $script:PathBind = '/bind'
    } else {
        $script:PathVol = 'C:\vol' ; $script:PathBind = 'C:\bind'
    }
}

function Get-Payload {
    <# Traduit une charge de test logique dans la syntaxe de l'interpreteur.
       Renvoie $null si la charge n'est pas realisable avec cet interpreteur. #>
    param([string]$Name, [int]$N = 0, [string]$Path = '', [string]$Dir = '')

    if ($script:ShKind -eq 'posix') {
        switch ($Name) {
            'probe'      { return 'echo DBENCH_OK' }
            'exit0'      { return 'exit 0' }
            'cpuburn'    { return 'while : ; do : ; done' }
            'sleep'      { return "sleep $N" }
            'memfill'    { return 'mkdir -p /ram; i=1; while [ $i -le 40 ]; do dd if=/dev/zero of=/ram/blk$i bs=1M count=64 2>/dev/null || exit 7; echo alloc=$((i*64))Mo; sleep 1; i=$((i+1)); done' }
            'iowrite'    { return "dd if=/dev/zero of=$Path bs=1M count=$N 2>/dev/null; sync" }
            'ioread'     { return "dd if=$Path of=/dev/null bs=1M 2>/dev/null" }
            'smallfiles' { return "mkdir -p $Dir; i=0; while [ `$i -lt $N ]; do echo x > $Dir/f`$i; i=`$((i+1)); done; sync" }
            'list'       { return "ls -la $Dir > /dev/null; find $Dir -type f | wc -l > /dev/null" }
        }
        return $null
    }

    if ($script:ShKind -eq 'pwsh') {
        switch ($Name) {
            'probe'      { return 'Write-Output DBENCH_OK' }
            'exit0'      { return 'exit 0' }
            'cpuburn'    { return 'while(1){}' }
            'sleep'      { return "Start-Sleep -Seconds $N" }
            'memfill'    { return "`$a=@(); for(`$i=1;`$i -le 40;`$i++){ `$a += ,(New-Object byte[] 67108864); Write-Output ('alloc=' + (`$i*64) + 'Mo'); Start-Sleep -Milliseconds 900 }" }
            'iowrite'    { return "`$b=New-Object byte[] 1048576; `$fs=[IO.File]::Create('$Path'); for(`$i=0;`$i -lt $N;`$i++){`$fs.Write(`$b,0,1048576)}; `$fs.Flush(`$true); `$fs.Close()" }
            'ioread'     { return "`$fs=[IO.File]::OpenRead('$Path'); `$b=New-Object byte[] 1048576; while(`$fs.Read(`$b,0,1048576) -gt 0){}; `$fs.Close()" }
            'smallfiles' { return "New-Item -ItemType Directory -Force -Path '$Dir' | Out-Null; for(`$i=0;`$i -lt $N;`$i++){ [IO.File]::WriteAllText((Join-Path '$Dir' ('f'+`$i)),'x') }" }
            'list'       { return "Get-ChildItem -Recurse '$Dir' | Measure-Object | Out-Null" }
        }
        return $null
    }

    if ($script:ShKind -eq 'cmd') {
        switch ($Name) {
            'probe'      { return 'echo DBENCH_OK' }
            'exit0'      { return 'exit 0' }
            'cpuburn'    { return 'for /l %i in (0,0,1) do @rem' }
            'sleep'      { return "ping -n $N 127.0.0.1 > nul" }
            'smallfiles' { return "if not exist $Dir mkdir $Dir & for /l %i in (1,1,$N) do @echo x > $Dir\f%i" }
            'list'       { return "dir /s /b $Dir > nul" }
        }
        return $null   # memoire et E/S sequentielles non mesurables en cmd pur
    }
    return $null
}

function Invoke-InImage {
    <# docker run avec l'interpreteur detecte #>
    param([string[]]$Pre = @(), [string]$Cmd, [string]$Ref, [int]$TimeoutSec = 120)
    if (-not $Ref) { $Ref = $Image }
    $a = @('run') + $Pre + @('--entrypoint', $script:ShExe, $Ref) + $script:ShArgs + @('"' + $Cmd + '"')
    return Invoke-Docker $a -TimeoutSec $TimeoutSec
}

function Invoke-InContainer {
    <# docker exec avec l'interpreteur detecte #>
    param([string]$Name, [string]$Cmd, [int]$TimeoutSec = 600)
    $a = @('exec', $Name, $script:ShExe) + $script:ShArgs + @('"' + $Cmd + '"')
    return Invoke-Docker $a -TimeoutSec $TimeoutSec
}

function Find-ImageShell {
    <# Renvoie le candidat retenu (hashtable) ou $null, et configure le runner. #>
    param([string]$Ref, [switch]$Verbose2)
    $saveKind = $script:ShKind
    foreach ($c in $script:ShellCandidates) {
        $script:ShKind = $c.Kind
        $probe = Get-Payload 'probe'
        $a = @('run','--rm','--entrypoint',$c.Exe,$Ref) + $c.Args + @('"' + $probe + '"')
        $r = Invoke-Docker $a -TimeoutSec 90
        if ($r.Code -eq 0 -and $r.Out -match 'DBENCH_OK') { Set-Runner $c ; return $c }
        if ($Verbose2) {
            $msg = (($r.Err -split "`n") | Where-Object { $_.Trim() } | Select-Object -First 1)
            Write-Host ("      {0,-24} [{1,-5}] code {2}  {3}" -f $c.Exe, $c.Kind, $r.Code, $msg) -ForegroundColor DarkGray
        }
    }
    $script:ShKind = $saveKind
    return $null
}

function Test-ImageShell {
    param([string]$Ref)
    return ($null -ne (Find-ImageShell -Ref $Ref))
}

function Test-ImagePresent {
    param([string]$Ref)
    $r = Invoke-Docker @('image','inspect','--format','{{.Id}}',$Ref) -TimeoutSec 60
    return ($r.Code -eq 0)
}

function Get-ImageOs {
    param([string]$Ref)
    $r = Invoke-Docker @('image','inspect','--format','{{.Os}}/{{.Architecture}}',$Ref) -TimeoutSec 60
...
if ($Image) {
    if (-not (Test-ImagePresent $Image)) {
        Write-Bad "L'image $Image est ABSENTE du cache local."
        Write-Bad "Le pull a echoue ou n'a jamais ete fait. Diagnostic :"
        Write-Bad "  docker pull $Image"
        Write-Bad "  (verifiez proxy/VPN, authentification au registre : docker login <registre>)"
        exit 1
    }
    Write-Ok "Image presente en local : $Image"
    if ($Shell) {
        $forced = @{ Kind = $ShellKind
                     Exe  = $Shell
                     Args = $(if ($ShellKind -eq 'cmd') { @('/c') }
                              elseif ($ShellKind -eq 'pwsh') { @('-NoProfile','-NonInteractive','-Command') }
                              else { @('-c') }) }
        Set-Runner $forced
        Write-Ok "Interpreteur impose : $Shell (mode $ShellKind) - detection ignoree"
        $probe = Invoke-InImage -Pre @('--rm') -Cmd (Get-Payload 'probe') -TimeoutSec 90
        if ($probe.Code -eq 0 -and $probe.Out -match 'DBENCH_OK') {
            Write-Ok "Verification reussie."
        } else {
            Write-Warn2 "La verification a echoue (code $($probe.Code)) : $(($probe.Err -split "`n")[0])"
            Write-Warn2 "Le test continue quand meme avec cet interpreteur."
        }
        $skipDetect = $true
    }
    if (-not $skipDetect) {
    Write-Info "Plateforme de l'image : $(Get-ImageOs $Image)"
    Write-Info "Recherche d'un interpreteur utilisable (sh, bash, busybox, powershell, cmd)..."
    $sh = Find-ImageShell -Ref $Image -Verbose2
    if (-not $sh) {
        Write-Bad "L'image $Image ne contient AUCUN interpreteur utilisable."
        Write-Bad "Ni sh/bash/busybox (Linux), ni powershell.exe/cmd.exe (Windows)."
        Write-Bad "Plateforme declaree par l'image : $(Get-ImageOs $Image)"
        Write-Bad "Il s'agit d'une image distroless, scratch ou durcie."
        Write-Bad "Solution : construire une variante de test a partir de cette image."
        Write-Bad "  Dockerfile :  FROM $Image"
        Write-Bad "                (couche outillage, ou COPY --from d'une image avec busybox)"
        Write-Bad "Sinon, utilisez -ImportTar .\rq-busybox-rootfs.tar pour mesurer la machine."
        exit 1
    }
    Write-Ok "Interpreteur retenu : $($sh.Exe) (mode $($sh.Kind))"
    }
} else {
    ...
    Write-Ok "Image retenue : $Image  (interpreteur $script:ShExe, mode $script:ShKind)"
}
...
$starts = @()
for ($i=1; $i -le 10; $i++) {
    $r = Invoke-InImage -Pre @('--rm','--name',"$script:Prefix-cs-$i") -Cmd (Get-Payload 'exit0') -TimeoutSec 120
...
for ($i=1; $i -le $cpuTargets; $i++) {
    [void](Invoke-InImage -Pre @('-d','--name',"$script:Prefix-cpu-$i",'--cpus','1') -Cmd (Get-Payload 'cpuburn') -TimeoutSec 60)
}
...
[void](Invoke-InImage -Pre @('-d','--name',"$script:Prefix-cpuhalf",'--cpus','0.5') -Cmd (Get-Payload 'cpuburn') -TimeoutSec 60)
...
$fillCmd = Get-Payload 'memfill'
if (-not $fillCmd) {
    Add-Finding 'INFO' 'Memoire' "Test de pression memoire non realisable : l'image ne fournit que cmd.exe." `
        "Utilisez une image disposant de powershell.exe (servercore) ou d'un shell POSIX pour cette mesure."
    $memRun = [pscustomobject]@{ Out=''; Err=''; Code=-1; Ms=0 }
} else {
$memPre = @('--name',"$script:Prefix-mem",'--memory',"${limitMB}m",'--memory-swap',"${limitMB}m")
if ($script:ShKind -eq 'posix') { $memPre += @('--tmpfs',"/ram:size=$([int]($limitMB*2))m") }
$memRun = Invoke-InImage -Pre $memPre -Cmd $fillCmd -TimeoutSec 240
...
$runIo = Invoke-InImage -Pre @('-d','--name',"$script:Prefix-io",
    '-v',"${script:Prefix}_vol:$($script:PathVol)",
    '-v',"`"$($script:BindPath):$($script:PathBind)`"") -Cmd (Get-Payload 'sleep' -N 900) -TimeoutSec 120
...
if ($runIo.Code -ne 0) {
    ...
    function Measure-Io {
        ...
        if (-not $Cmd) { return [pscustomobject]@{ Label=$Label; Ms=0; Ok=$false; Err='non applicable' } }
        $r = Invoke-InContainer -Name "$script:Prefix-io" -Cmd $Cmd -TimeoutSec $Timeout
        return [pscustomobject]@{ Label=$Label; Ms=$r.Ms; Ok=($r.Code -eq 0); Err=$r.Err }
    }

    Write-Info "Ecriture sequentielle de $IoSizeMB Mo..."
    $wVol  = Measure-Io 'vol-write'  (Get-Payload 'iowrite' -N $IoSizeMB -Path "$($script:PathVol)/f".Replace('\/','\'))
    $wBind = Measure-Io 'bind-write' (Get-Payload 'iowrite' -N $IoSizeMB -Path "$($script:PathBind)/f".Replace('\/','\'))
    Write-Info "Lecture sequentielle..."
    $rVol  = Measure-Io 'vol-read'   (Get-Payload 'ioread' -Path "$($script:PathVol)/f".Replace('\/','\'))
    $rBind = Measure-Io 'bind-read'  (Get-Payload 'ioread' -Path "$($script:PathBind)/f".Replace('\/','\'))
    Write-Info "Creation de $SmallFiles petits fichiers (metadonnees)..."
    $sVol  = Measure-Io 'vol-small'  (Get-Payload 'smallfiles' -N $SmallFiles -Dir "$($script:PathVol)/s".Replace('\/','\'))
    $sBind = Measure-Io 'bind-small' (Get-Payload 'smallfiles' -N $SmallFiles -Dir "$($script:PathBind)/s".Replace('\/','\'))
    Write-Info "Parcours de l'arborescence (stat)..."
    $lVol  = Measure-Io 'vol-list'   (Get-Payload 'list' -Dir "$($script:PathVol)/s".Replace('\/','\'))
    $lBind = Measure-Io 'bind-list'  (Get-Payload 'list' -Dir "$($script:PathBind)/s".Replace('\/','\'))

    if (-not $wVol.Ok -and -not $sVol.Ok) {
        Add-Finding 'INFO' 'Disque' "Tests d'E/S non realisables avec cet interpreteur (cmd.exe seul)." `
            "Rejouez le test avec une image fournissant powershell.exe ou un shell POSIX."
    }
...
for ($i=1; $i -le $MaxContainers; $i++) {
    $r = Invoke-InImage -Pre @('-d','--name',"$script:Prefix-d-$i") -Cmd (Get-Payload 'sleep' -N 600) -TimeoutSec 120
...
[void]$md.AppendLine("| Interpreteur | $script:ShExe (mode $script:ShKind) |")
[void]$md.AppendLine("| Plateforme image | $(Get-ImageOs $Image) |")
